...
def load_from_dir(dirpath, remove_original=False):
    ...
    openmm_filename = "ion_trajectories_openmm.???" # TODO: openmm filename
...
def import_from_comsol_file(filepath):
    ...
    for i, index in enumerate(ion_indices):
        ...
        mass_kg = ions_mass[i] * scipy.constants.atomic_mass  # Convert atomic mass unit to kg
...
class IonTrajectories:
    ...
    def save(self, dirpath):
        ...
        """
        np.savez_compressed(os.path.join(dirpath, _load_save_filename),
                            poses=self.poses,
                            vels=self.vels,
                            ts=self.ts,
    ...
    def cylindrical_to_cartesian(cylindrical_coords):
        ...
        """
        r, phi, z = cylindrical_coords[:, :, 0], cylindrical_coords[:, :, 1], cylindrical_coords[:, :, 2]
        x = r * np.cos(phi)
        y = r * np.sin(phi)
    ...
    def plot_initial_velocity_distribution(self, ax):
        ...
        ax.hist(velocity_norms, bins=10, color='blue', alpha=0.7)
    ...
    def plot_initial_velocity_angle_distribution(self, ax):
        ...
        initial_velocities = self.vels[:, 0, :]
        velocity_norms = np.linalg.norm(initial_velocities, axis=1)
        angles = np.arccos(initial_velocities[:, 2] / velocity_norms)  # Calculate angle with z-axis
    ...
    def plot_trajectories_cartesian(self, ax, cmap):
        ...
        for i, color in enumerate(colors):
            ax.plot(poses_cartesian[i, :, 0], poses_cartesian[i, :, 1], poses_cartesian[i, :, 2], color=color, alpha=0.25)
        ax.set_xlabel('x (cm)')
        ax.set_ylabel('y (cm)')
        ax.set_zlabel('z (cm)')
    ...
    def plot_trajectories_xy(self, ax, cmap):
        ...
        poses_cartesian = self.cylindrical_to_cartesian(self.poses)
        for i, color in enumerate(colors):
            ax.plot(poses_cartesian[i, :, 0], poses_cartesian[i, :, 1], color=color, alpha=0.25)
        ax.set_xlabel('x (cm)')
        ax.set_ylabel('y (cm)')
    ...
    def plot_all(self):
        ...
        layout = [["3D", "XY",   "VelNorm"],
                  ["3D", "ZPhi", "VelAngle"]]
        fig, axd = plt.subplot_mosaic(layout, figsize=np.array((3, 2))*4, per_subplot_kw={"3D":{"projection":"3d"}}, layout="constrained")
        ...
        sm.set_array([])
        cbar = fig.colorbar(sm, ax=[axd["3D"]], orientation='vertical', fraction=0.02, pad=0.04, location="left")
